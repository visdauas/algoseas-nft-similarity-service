...
export function assetFromJSON(
  ...
  const asset: Asset = {
    ...
    forSale: hasMarketActivity
      ? marketActivity.event.includes('ACTIVE_LISTING')
      : false,
...
export async function getAsset(assetId: number): Promise<Asset | undefined> {
  ...
  if (trxInformation.transactions == undefined) {
    return assetFromJSON(
      assetId,
      assetInformation.assetInformation,
      assetInformation.marketActivity[0],
    );
  }
  const lastSoldRound =
    trxInformation.transactions[0].confirmedRound == undefined
      ? 0
      : trxInformation.transactions[0].confirmedRound;
  return assetFromJSON(
    assetId,
    assetInformation.assetInformation,
    assetInformation.marketActivity[0],
...
export function isValidAssetData(assetInformation: any): boolean {
  if (
    ...
    assetInformation.assetInformation.nProps.properties.constitution ==
      undefined ||
    assetInformation.assetInformation.nProps.properties.luck == undefined ||
